...
ASSUMPTIONS AND LIMITATIONS
	The assumptions and limitations on my program include the following:
		...
		The program assumes that the user does not have any pre-existing investments prior to using the program.
		...
		Upon ending of the program investments still in the portfolio are not saved.
...
TEST PLAN
	...
	Testing the buy method: enter symbol
		Entering nothing, or a string: symbol is accepted
...
POSSIBLE IMPROVEMENTS
	Drastically decrease the length of the program's code by implementing better more concise methods. 
